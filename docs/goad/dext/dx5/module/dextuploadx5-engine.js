...
DX5FileItem.prototype.update = function() {
    if (this.onUpdated && typeof this.onUpdated === "function") {
        this.onUpdated(this)
    }
};
DX5FileItem.prototype.toVirtual = function(openUrl, downUrl) {
...
DX5FileItem.prototype.toString = function() {
    return "[object DX5FileItem]"
};
DX5FileItem.prototype.toContentString = function() {
    return this.id + " " + this.type + " " + this.status + " " + this.checked
};
var DX5FileList = function() {
    Array.call(this);
    this.totalSizeA = 0;
    this.totalSizeL = 0;
    this.virtualLength = 0
};
DX5FileList.prototype = Object.create(Array.prototype);
DX5FileList.prototype.update = function() {
    this.totalSizeA = 0;
    this.totalSizeL = 0;
    this.virtualLength = 0;
    for (var i = 0; i < this.length; i++) {
        if (false == (this[i].toString() == "[object DX5FileItem]")) continue;
        if (this[i].size > 0) this.totalSizeA += this[i].size;
        if ("FILE" == this[i].type && this[i].ofile) {
            this.totalSizeL += this[i].ofile.size
        } else if ("VIRTUAL" == this[i].type) {
            this.virtualLength++
        }
    }
};
DX5FileList.prototype.onchanged = function(_ev) {
    this.update()
};
DX5FileList.prototype.push = function(f) {
    if (f.toString() == "[object DX5FileItem]") {
        ...
        console.log("f is not DX5FileItem => " + f.toString())
    }
};
DX5FileList.prototype.pop = function() {
    var f = null;
    var notfound = true;
    while (notfound) {
        f = Array.prototype.pop.call(this);
        if (!f) return null;
        if (f.toString() == "[object DX5FileItem]") {
            this.onchanged({
                state: "delete",
...
DX5FileList.prototype.shift = function() {
    var f = null;
    var nofound = true;
    while (nofound) {
        f = Array.prototype.shift.call(this);
        if (!f) return null;
        if (f.toString() == "[object DX5FileItem]") {
            this.onchanged({
                state: "shift",
...
DX5FileList.prototype.getTotalSize = function(onlyLocal) {
    return onlyLocal === true ? this.totalSizeL : this.totalSizeA
};
DX5FileList.prototype.getLength = function(onlyLocal) {
    return onlyLocal === true ? this.length - this.virtualLength : this.length
};
DX5FileList.prototype.getItem = function(id) {
    var target = null;
    for (var i = 0; i < this.length; i++) {
        if (id == this[i].id) {
            target = this[i];
...
DX5FileList.prototype.removeById = function(id) {
    var t = null;
    for (var i = 0; i < this.length; i++) {
        t = this[i];
        if (id == t.id) {
...
DX5FileList.prototype.removeByIndex = function(index) {
    var t = null;
...
DX5FileList.prototype.removeByObject = function(obj) {
    var t = null;
    for (var i = 0; i < this.length; i++) {
        t = this[i];
        if (obj == t) {
...
DX5FileList.prototype.clear = function(flag) {
...
var DX5UploadStatus = function() {
    this.xhr = null;
    this.totalSize = 0;
    this.totalCount = 0;
    this.totalSendSize = 0;
    this.totalPrevSendSize = 0;
    this.completeCount = 0;
    this.uploadStartTime = null;
    this.uploadCountTime = 0;
    this.currentStartTime = null;
    this.currentCountTime = 0;
    this.chunkedStartTime = null;
    this.chunkedCountTime = 0;
    this.currentItem = null;
    this.currentName = "";
    this.currentSize = 0;
    this.currentSendSize = 0;
    this.currentChunkedAction = "P";
    this.currentChunkedSize = 0;
    this.currentChunkedPrevSendSize = 0;
    this.currentLocalKey = "";
    this.currentServerKey = "";
    this.currentServerSize = 0;
    this.currentResponseValue = ""
};
DX5UploadStatus.prototype.clear = function() {
    this.totalSize = this.totalCount = this.totalSendSize = this.totalPrevSendSize = this.completeCount = 0;
    this.uploadStartTime = null;
    this.currentStartTime = null;
    this.currentItem = null;
    this.currentName = "";
    this.currentSize = this.currentSendSize = 0;
    this.currentChunkedAction = "P";
    this.currentChunkedSize = 0;
    this.currentChunkedPrevSendSize = 0;
    this.currentLocalKey = "";
...
DX5UploadStatus.prototype.clearItem = function() {
    this.currentStartTime = null;
    this.currentItem = null;
    this.currentName = "";
    this.currentSize = this.currentSendSize = 0;
...
DX5UploadStatus.prototype.getTotalRate = function() {
    if (this.totalSize == 0) return 100;
    else return parseFloat((this.totalSendSize * 100 / this.totalSize).toFixed(1))
};
DX5UploadStatus.prototype.getTotalSpeed = function() {
    var diff = this.uploadCountTime + this.currentCountTime + this.chunkedCountTime;
    var sec = (diff - diff % 1e3) / 1e3;
    return parseFloat(sec == 0 ? this.totalSendSize.toFixed(1) : (this.totalSendSize / sec).toFixed(1))
};
DX5UploadStatus.prototype.getTotalSendTime = function() {
    return this.uploadStartTime ? Date.now() - this.uploadStartTime : 0
};
DX5UploadStatus.prototype.getCurrentRate = function() {
    if (this.currentSize == 0) return 100;
    else return parseFloat((this.currentSendSize * 100 / this.currentSize).toFixed(1))
};
DX5UploadStatus.prototype.getCurrentSendTime = function() {
    return this.currentStartTime ? Date.now() - this.currentStartTime : 0
};
DX5UploadStatus.prototype.getCurrentSpeed = function() {
    var diff = this.currentCountTime + this.chunkedCountTime;
    var sec = (diff - diff % 1e3) / 1e3;
    return parseFloat(sec == 0 ? this.currentSendSize.toFixed(1) : (this.currentSendSize / sec).toFixed(1))
};
DX5UploadStatus.prototype.getRemainedTotalTime = function() {
    var remainedSize = this.totalSize - this.totalSendSize;
    var speed = this.getTotalSpeed();
    if (remainedSize > 0) {
        return speed == 0 ? Infinity : remainedSize * 1e3 / speed
    } else {
        return 0
    }
};
DX5UploadStatus.prototype.getRemainedCurrentTime = function() {
    var remainedSize = this.currentSize - this.currentSendSize;
...
var DX5DownloadStatus = function() {
    this.xhr = null;
    this.totalSize = 0;
    this.totalCount = 0;
    this.totalSendSize = 0;
...
DX5DownloadStatus.prototype.clear = function() {
    this.totalSize = this.totalCount = this.totalSendSize = this.totalPrevSendSize = this.completeCount = 0;
    this.downloadStartTime = null;
    this.currentStartTime = null;
    this.currentItem = null;
...
DX5DownloadStatus.prototype.clearItem = function() {
    this.currentStartTime = null;
    this.currentItem = null;
    this.currentName = "";
    this.currentSize = this.currentSendSize = 0
};
DX5DownloadStatus.prototype.getTotalRate = function() {
    if (this.totalSize == 0) return 100;
    else return parseFloat((this.totalSendSize * 100 / this.totalSize).toFixed(1))
};
DX5DownloadStatus.prototype.getTotalSpeed = function() {
    var diff = this.downloadCountTime + this.currentCountTime;
    var sec = (diff - diff % 1e3) / 1e3;
    return parseFloat(sec == 0 ? this.totalSendSize.toFixed(1) : (this.totalSendSize / sec).toFixed(1))
};
DX5DownloadStatus.prototype.getTotalSendTime = function() {
    return this.downloadStartTime ? Date.now() - this.downloadStartTime : 0
};
DX5DownloadStatus.prototype.getCurrentRate = function() {
    if (this.currentSize == 0) return 100;
    else return parseFloat((this.currentSendSize * 100 / this.currentSize).toFixed(1))
};
DX5DownloadStatus.prototype.getCurrentSendTime = function() {
    return this.currentStartTime ? Date.now() - this.currentStartTime : 0
};
DX5DownloadStatus.prototype.getCurrentSpeed = function() {
    var diff = this.currentCountTime;
    var sec = (diff - diff % 1e3) / 1e3;
    return parseFloat(sec == 0 ? this.currentSendSize.toFixed(1) : (this.currentSendSize / sec).toFixed(1))
};
DX5DownloadStatus.prototype.getRemainedTotalTime = function() {
    var remainedSize = this.totalSize - this.totalSendSize;
    var speed = this.getTotalSpeed();
    if (remainedSize > 0) {
        return speed == 0 ? Infinity : remainedSize * 1e3 / speed
...
DX5DownloadStatus.prototype.getRemainedCurrentTime = function() {
    var remainedSize = this.currentSize - this.currentSendSize;
    var speed = this.getCurrentSpeed();
    if (remainedSize > 0) {
        return speed == 0 ? Infinity : remainedSize * 1e3 / speed
...
DX5FileEngine.prototype = function() {
    var LS = window.localStorage;

    function isDataURL(s) {
        return new RegExp("^data:.+/.+;.+,.*").test(s)
    }

    function isHttpURL(s) {
        return new RegExp("^https?://", "i").test(s)
    }

    function d2b(w, s, m) {
        var B = w.Blob || w.MozBlob || w.WebKitBlob || function(s) {
            return String(s)
        };
        B = B.call ? B.bind(w) : B;
        if (isDataURL(s)) {
            var p = s.split(/[:;,]/),
                t = p[1],
                fdec = p[2] == "base64" ? atob : decodeURIComponent,
                bin = fdec(p.pop()),
                blen = bin.length,
                uia = new Uint8Array(blen);
            for (var i = 0; i < blen; i++) uia[i] = bin.charCodeAt(i);
            return new B([uia], {
                type: t
            })
        } else {
            return s instanceof B ? s : new B([s], {
                type: m
            })
        }
    }

    function saveData(data, f, m, isSafari, scb, ecb) {
        if (isHttpURL(data)) {
            ecb("ESVG-00068", "_{0}은(는) 지원하지 않는 파일 다운로드 프로토콜입니다.", [data]);
            return
        }
        var w = window,
            d = w.document,
        ...
        f = f || "download";
        scb = typeof scb === "function" ? scb : function() {};
        ecb = typeof ecb === "function" ? ecb : function(_ecode, _emsg, _eparams) {};
        if (w.navigator.msSaveBlob) {
            blob = d2b(w, data, m);
            if (w.navigator.msSaveBlob(blob, f)) scb();
            else ecb()
        } else if (u === true) {
            blob = d2b(w, data, m);
            ...
            ecb("ESVG-00067", "_대상 브라우저는 다중 파일 다운로드를 지원하지 않습니다.", undefined)
        }
    }
    ...
    var createFI = function(ctrId, id, file, vindex, type, name, size, meta) {
    ...
    var attachORAFEvents = function(engine, xhr) {
        ...
        xhr.addEventListener("abort", function(_e) {
            engine.acting = false;
            if (engine.oncancel && typeof engine.oncancel == "function") {
                engine.oncancel()
            }
        }, false);
        xhr.addEventListener("timeout", function(_e) {
            engine.acting = false;
    ...
    var attachEXNJEvents = function(engine, xhr) {
        ...
        xhr.upload.addEventListener("progress", function(e) {
            if (engine.upstatus.currentChunkedAction != "P" && e.lengthComputable && engine.onprogress && typeof engine.onprogress == "function") {
    ...
    var downloadURI = function(engine, url, name) {
        var oAnchor = engine.downObjects.button;
        if ("download" in oAnchor) {
            ...
            var oframe = engine.downObjects.iframe;
            oframe.src = url
        }
    };
    var downloadSingle = function(engine, target) {
        try {
            AJAX(target.downUrl, "head", {
                load: function(_res) {
                    downloadURI(engine, target.downUrl, target.name)
                },
                abort: function() {},
    ...
    var execute = function(engine, target) {
        ...
        var pop = window.open(target.openUrl, "DEXTUploadX5-" + target.id, popWinOption);
    ...
    var validateURLInDomain = function(engine, authkey, url) {
        ...
        if (lic == "0") {
            ...
            permitcnt = max = 2, isultimate = true
        } else if (lic == "4") {
            permitcnt = max = 10;
            return true
        } else if (lic == "5") {
        ...
        for (i = 0, len = Math.min(domains.length, permitcnt), cdomain = null, mi = -1; i < len; i++) {
        ...
        throwError(engine, "ESVG-00060", "_{0}: 등록된 도메인(IP)이 아니다.", [upperDomain]);
        return false
    };
    var validateURLInCombination = function(engine, authkey, url) {
        ...
        if (upperDomain == "LOCALHOST" || upperDomain == "127.0.0.1") return true;
        for (i = 0, cdomain = null, mi = -1; i < max; i++) {
    ...
    var authenticate = function(engine, auth, url) {
        ...
        if (isError === false && ["PID", "SN"].filter(function(v, _i) {
                return decoded.toUpperCase().indexOf(v) >= 0
            }).length === 2) {
    ...
    return {
        addFile: function(file) {
            ...
            if (isFile(file)) {
                ...
                item = createFI(engine.controlId, createId(++engine.cumulative), blob, file.vindex || "", "VIRTUAL", file.name, typeof file.size === "undefined" ? -1 : adjustInteger(file.size, false, -1), file.meta);
                item.ext = getFileExtension(file.name);
        ...
        upload: function(ids) {
            ...
            if (engine.acting == true) {
                throwError(engine, "ESVG-00024", "_파일을 전송하고 있는 상태이다.");
                return
            }
            if (!engine.uploadUrl) {
            ...
            if (!engine.licenseKey) {
                throwError(engine, "ESVG-00035", "인증키가 없습니다. 파일 업로드 작업은 중단된다.");
                return
            }
            ...
            if (engine.uploadMode == "ORAF") transferORAF(engine);
                ...
                engine.acting = false;
                throwError(engine, "ESVG-00069", "_{0}은(는) 올바른 파일 업로드 방식이 아니다.", [engine.uploadMode])
            }
        },
